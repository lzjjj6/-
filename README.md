...
![直线拟合算法对比](line_fitting.svg)

### 圆拟合对比

![圆拟合算法对比](circle_fitting.svg)

### 差分隐私加噪前后（直线）

![原始点集 vs 隐私化点集](orig%20vs%20privacy_points_line.svg)

### 原始 vs DP vs PP-DBSCAN（RANSAC · 直线）

![三路拟合对比](orig%20vs%20dp%20vs%20ppdbscan_ransac_line.svg)

### 隐私-精度权衡

![隐私预算与 F1 关系](privacy_tradeoff.svg)

> 运行 `python main.py` 后会在项目根目录生成对应的 `.png` 图表；仓库中已附带部分 `.svg` 预览图。
...
- 差分隐私: Dwork et al., 2006  
- PP-DBSCAN 思路参见 `dbscan_privacy.py` 模块注释  
